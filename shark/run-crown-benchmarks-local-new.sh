...
for model in "${MODELS_TO_TEST[@]}"; do
    ...
    for eps in "${EPS_LIST[@]}"; do
        ...
        mkdir -p crown-results/$model/eps_$eps
        ...
        for sample in "${IMAGE_LIST[@]}"; do
            ...
            for current_mode in "${MODES_TO_RUN[@]}"; do
                ...
                # 检查是否已有任意线程的dealer结果，有则跳过预处理
                DEALER_DONE=false
                for t in "${THREADS_LIST[@]}"; do
                    if [ -f "crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}_t${t}_dealer.txt" ]; then
                        DEALER_DONE=true
                        break
                    fi
                done
                # 兼容检查无线程后缀的旧dealer文件
                if [ -f "crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}_dealer.txt" ]; then
                    DEALER_DONE=true
                fi
                ...
                if [ "$DEALER_DONE" = false ]; then
                    ...
                    ./build/benchmark-crown 2 $CUSTOM_ARGS &> crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}_dealer.txt
                    ...
                    echo "[$(date +'%H:%M:%S')] DEALER preprocessing already done, skipping..."
                    # 重新获取文件大小用于后续summary
                    if [ -f "server.dat" ]; then
                        SERVER_SIZE=$(stat -c%s server.dat)
                        SERVER_HUMAN=$(bytes_to_human $SERVER_SIZE)
                ...
                for THREADS in "${THREADS_LIST[@]}"; do
                    ...
                    # 检查是否已存在结果
                    if [ -f "crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}${THREAD_SUFFIX}_summary.txt" ]; then
                        echo "    [SKIP] Results for EPS=$eps, Image=$IMAGE_ID, Mode=$MODE_DISPLAY, Threads=$THREADS already exist"
                        echo "    Quick preview:"
                        tail -5 "crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}${THREAD_SUFFIX}_summary.txt" | sed 's/^/    /'
                        echo ""
                        continue
                    fi
                    ...
                    # 先启动 SERVER（在后台）
                    if [ "$THREADS" = "0" ]; then
                        ./build/benchmark-crown 0 $IP_ADDRESS $CUSTOM_ARGS &> crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}${THREAD_SUFFIX}_server.txt &
                    else
                        OMP_NUM_THREADS=$THREADS ./build/benchmark-crown 0 $IP_ADDRESS $CUSTOM_ARGS &> crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}${THREAD_SUFFIX}_server.txt &
                    fi
                    SERVER_PID=$!
                    ...
                    # 启动 CLIENT
                    if [ "$THREADS" = "0" ]; then
                        ./build/benchmark-crown 1 $IP_ADDRESS $CUSTOM_ARGS &> crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}${THREAD_SUFFIX}_client.txt
                    else
                        OMP_NUM_THREADS=$THREADS ./build/benchmark-crown 1 $IP_ADDRESS $CUSTOM_ARGS &> crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}${THREAD_SUFFIX}_client.txt
                    fi
                    CLIENT_EXIT_CODE=$?
                    ...
                    if [ $CLIENT_EXIT_CODE -ne 0 ]; then
                        ...
                        echo "    Server output:"
                        cat crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}${THREAD_SUFFIX}_server.txt | sed 's/^/    /'
                        echo ""
                        echo "    Client output:"
                        cat crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}${THREAD_SUFFIX}_client.txt | sed 's/^/    /'
                    fi
                    ...
                    # 创建摘要
                    {
                        ...
                        grep -E "(MPC LB:|MPC UB:|End_to_end_time|crown_calculation)" crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}${THREAD_SUFFIX}_client.txt || echo "  (No results found - check for errors)"
                    } > crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}${THREAD_SUFFIX}_summary.txt

                    echo ""
                    cat crown-results/$model/eps_$eps/image_${IMAGE_ID}${MODE_SUFFIX}${THREAD_SUFFIX}_summary.txt | sed 's/^/    /'
                    echo ""
    ...
    # 创建模型总结 (修改版：扫描磁盘上所有存在的 EPS 和 Image 结果，区分恶意和半诚实以及线程数)
    {
        ...
        echo ""
        echo "--- All Results Found in crown-results/$model ---"

        # 1. 查找所有 eps_ 开头的目录，并按版本号排序
        EXISTING_EPS_DIRS=$(find "crown-results/$model" -maxdepth 1 -type d -name "eps_*" | sort -V)
                    ...
                    for img_id in $ALL_IMAGE_IDS; do
                        ...
                        if [ -n "$MAL_FILES" ]; then
                            ...
                            for mal_file in $MAL_FILES; do
                                ...
                                grep -E "(MPC LB:|MPC UB:|End_to_end_time)" "$mal_file" 2>/dev/null | sed 's/^/      /' || echo "      (No results content)"
                            done
                        fi
                        ...
                        if [ -n "$SH_FILES" ]; then
                            ...
                            for sh_file in $SH_FILES; do
                                ...
                                echo "    Threads=$thread_num:"
                                grep -E "(MPC LB:|MPC UB:|End_to_end_time)" "$sh_file" 2>/dev/null | sed 's/^/      /' || echo "      (No results content)"
                            done
                        fi
                        ...
                        if [ -f "$OLD_MAL_FILE" ]; then
                            ...
                            grep -E "(MPC LB:|MPC UB:|End_to_end_time)" "$OLD_MAL_FILE" 2>/dev/null | sed 's/^/    /' || echo "    (No results content)"
                        fi

                        OLD_SH_FILE="$eps_dir/image_${img_id}_sh_client.txt"
                        if [ -f "$OLD_SH_FILE" ]; then
                            echo "  [Semi-Honest Mode - Legacy (no thread info)]"
                            grep -E "(MPC LB:|MPC UB:|End_to_end_time)" "$OLD_SH_FILE" 2>/dev/null | sed 's/^/    /' || echo "    (No results content)"
                        fi
                        ...
                        if [ -f "$OLD_FILE" ]; then
                            ...
                            grep -E "(MPC LB:|MPC UB:|End_to_end_time)" "$OLD_FILE" 2>/dev/null | sed 's/^/    /' || echo "    (No results content)"
                        fi
    ...
    } > crown-results/$model/model_summary.txt
done

echo ""
echo "=========================================================="
echo "All tests completed! Results saved in: crown-results/"
echo "=========================================================="