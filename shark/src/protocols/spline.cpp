...
namespace shark
{
    namespace protocols
    {
        namespace spline
        {
            ...
            void gen_sh(int bin, int degree, const shark::span<u64> &X, shark::span<u64> &Y)
            {
                always_assert(X.size() == Y.size());
                ...
                send_sh_dcfring(Xneg, bin);

                shark::span<u64> r_selected_poly((degree + 1) * X.size());
                randomize(r_selected_poly);
                send_sh_ashare(r_selected_poly);
            ...
            void eval_sh(int bin, int degree, const std::vector<u64> &knots, const std::vector<u64> &polynomials, const shark::span<u64> &X, shark::span<u64> &Y)
            {
                u64 m = knots.size();
                u64 p = polynomials.size();
                always_assert((m + 1) * (degree + 1) == p);
                ...
                auto dcfkeys = recv_sh_dcfring(X.size(), bin);
                auto r_selected_poly = recv_sh_ashare((degree + 1) * X.size());
                auto S_share = recv_sh_ashare(X.size() * (degree + 1));
                auto T_share = recv_sh_ashare(X.size() * (degree + 1));

                shark::span<u64> selected_poly_share((degree + 1) * X.size());
                for (u64 i = 0; i < (degree + 1) * X.size(); ++i)
                {
                    selected_poly_share[i] = r_selected_poly[i];
                }
                ...
                for (u64 i = 0; i < X.size(); i++)
                {
                    ...
                    if (bin != 64)
                        idx_prev = idx_prev % (1ull << bin);
                    auto t_prev = dcfring_eval_sh(party, dcfkeys[i], idx_prev);
                    auto t_0 = t_prev;
                    for (u64 j = 0; j < m + 1; j++)
                    {
                        u64 t;
                        ...
                        if (j == m)
                            ...
                            t = dcfring_eval_sh(party, dcfkeys[i], idx);
                        auto s = t_prev - t;
                ...
                auto selected_poly = sh_reconstruct(selected_poly_share);
            ...
            void call(int bin, int degree, const std::vector<u64> &knots, const std::vector<u64> &polynomials, const shark::span<u64> &X, shark::span<u64> &Y)
            {
                ...
                {
                    if (semi_honest_mode)
                        eval_sh(bin, degree, knots, polynomials, X, Y);
                    else
