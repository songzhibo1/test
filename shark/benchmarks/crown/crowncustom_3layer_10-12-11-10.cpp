...
bool enable_debug = true;  // 这就是报错缺失的变量
...
// ==================== 核心算子 (Newton Reciprocal & Alpha) ====================

// 牛顿迭代求倒数: x_{n+1} = x_n * (2 - a * x_n)
...
// 计算自适应 Alpha: alpha = ReLU(U) / (ReLU(U) + ReLU(-L) + epsilon)
...
std::vector<double> b3_data = {
    0.02, 0.03, 0.19, 0.1, -0.06, -0.09, 0.05, -0.13, 0.18, 0.12
};
...
int main(int argc, char **argv) {
    ...
    // 定义 3 层网络结构 (对应 3 个权重矩阵)
    // 结构: Input(10) -> 12 -> 11 -> Output(10)
    std::vector<int> layer_dims = {10, 12, 11, 10};
    int num_layers = 3;
    int input_dim = 10;
    int output_dim = 10;

    float eps = 0.2;
    int true_label = 3;   // 修改为 3
    int target_label = 2; // 修改为 2
