...
int main(int argc, char **argv) {
    ...
    // --- 1. 定义模型名称与基础配置 ---
//    std::string model_name = "vnncomp_mnist_3layer_relu_256_best"; // 在此处定义模型名称
//    int input_dim = 784;
//    int output_dim = 10;
//    std::vector<int> layer_dims = {784, 256, 256, 10};
//    int num_layers = 3;
//    int true_label = 7;
//    int target_label = 3;
    std::string model_name = "vnncomp_mnist_5layer_relu_256_best"; // 在此处定义模型名称
    int input_dim = 784;
    int output_dim = 10;
    std::vector<int> layer_dims = {784, 256, 256, 256, 256, 10};
    int num_layers = 5;
    int true_label = 7;
    int target_label = 8;

//    std::string model_name = "vnncomp_mnist_7layer_relu_256_best"; // 在此处定义模型名称
//    int input_dim = 784;
//    int output_dim = 10;
//    std::vector<int> layer_dims = {784, 256, 256, 256, 256, 256, 256, 10};
//    int num_layers = 7;
//    int true_label = 7;
//    int target_label = 3;
    ...
    std::vector<float> eps_list = {0.01, 0.02, 0.05, 0.1, 0.2, 0.5};
//    std::vector<float> eps_list = {0.01,};


    // 解析命令行参数（允许通过 --model 覆盖）
    for (int i = 1; i < argc; ++i) {
        std::string arg = argv[i];
        if (arg.find("--model=") == 0) {
            model_name = arg.substr(8);
            base_path = "shark_crown_ml/crown_mpc_data/" + model_name;
            weights_file = base_path + "/weights/weights.dat";
            input_file = base_path + "/images/0.bin";
        }
    ...
    shark::span<u64> epsilon_share(1), one_share(1), two_share(1), diff_vec(output_dim), x0(input_dim), ones_input(input_dim);
    if (party == CLIENT) {
    ...
    // --- 3. 批量测试循环 ---
    for (float current_eps : eps_list) {
        if (enable_debug) debug_records.clear();
        shark::span<u64> eps_share(1);
        if (party == CLIENT) eps_share[0] = float_to_fixed(current_eps);
        ...
        if (party != DEALER) {
            std::cout << "\n============================================" << std::endl;
            std::cout << "MODEL: " << model_name << std::endl; // 此处指明模型
            std::cout << "EPS: " << current_eps << " | True: " << true_label << " | Target: " << target_label << std::endl;
            std::cout << "--------------------------------------------" << std::endl;
