 ...
 * 2. 延迟截断策略 - 减少截断次数
 * 3. 使用 i128 进行本地累加计算
 * 4. 更精确的 reciprocal (5次 Newton 迭代)
 * 5. 分段计算避免溢出
 */
...
using u64 = shark::u64;
using i64 = int64_t;
using i128 = __int128;  // 128位整数用于本地高精度计算
using namespace shark::protocols;

// ==================== 精度配置 ====================
// 使用 f=20 提供最大动态范围
// 乘法后最大值约 2^40，远离 2^64 溢出边界
const int f = 20;
const u64 SCALAR_ONE = 1ULL << f;

// 用于延迟截断的辅助精度
const int f_high = 30;  // 高精度中间计算
const int f_diff = f_high - f;  // 精度差
...
struct DebugRecord {
    std::string name;
    shark::span<u64> data;
    DebugRecord(const std::string& n, shark::span<u64>& d) : name(n) {
        data = shark::span<u64>(d.size());
        for (size_t i = 0; i < d.size(); ++i) data[i] = d[i];
    }
};
std::vector<DebugRecord> debug_records;
void record_value(const std::string& name, shark::span<u64>& data) {
    if (enable_debug) debug_records.emplace_back(name, data);
}
...
int main(int argc, char **argv) {
    ...
    if (party != DEALER) {
        std::cout << "\n********************************************" << std::endl;
        std::cout << "  CROWN V4 (High Precision with i128)" << std::endl;
        std::cout << "  MODEL: " << model_name << std::endl;
        std::cout << "  Layers: " << num_layers << ", Hidden: " << hidden_dim << std::endl;
        std::cout << "  Precision: f=" << f << " (uses i128 for local accumulation)" << std::endl;
        std::cout << "  Newton iterations: 5" << std::endl;
